...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "bhuvy-rdata", "tests")); system.time(test_check("bhuvy-rdata", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran bhuvy-rdata_$(VERSION).tar.gz
	rm -rf bhuvy-rdata.Rcheck/
